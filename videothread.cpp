...
void VideoThread::process()
{
    ...
    if(!frame_to_resize.empty()){
        ...
        if(disp_selected_pcb){
            cv::rectangle(frame_cv, selected_pcb, cv::Scalar(51,255,51), 1);
            cv::putText(frame_cv, "Selected PCB", cv::Point(selected_pcb.x+selected_pcb.width+5, selected_pcb.y+selected_pcb.height+5), cv::FONT_HERSHEY_COMPLEX, 0.25, cv::Scalar(51,255,51),1);
        }
        if(disp_scan_height_point){

            cv::circle(frame_cv, scan_height_point, 3, cv::Scalar(0,0,255), 1);
            cv::putText(frame_cv, "Point for height measurement", cv::Point(scan_height_point.x+6, scan_height_point.y-6), cv::FONT_HERSHEY_COMPLEX, 0.25, cv::Scalar(255,0,0),1);
        }

        if(disp_pcb_corner_point){
            cv::circle(frame_cv, pcb_corner_point, 3, cv::Scalar(0,0,255), 1);
            cv::putText(frame_cv, "PCB corner", cv::Point(pcb_corner_point.x+6, pcb_corner_point.y-6), cv::FONT_HERSHEY_COMPLEX, 0.25, cv::Scalar(255,0,0),1);
        }

        if(disp_pcb_ref_point){

            cv::circle(frame_cv, pcb_ref_point, 3, cv::Scalar(0,0,255), 1);
            cv::putText(frame_cv, "PCB reference point", cv::Point(pcb_ref_point.x+6, pcb_ref_point.y-6), cv::FONT_HERSHEY_COMPLEX, 0.25, cv::Scalar(255,0,0),1);
        }

        //cv::circle(frame_cv, cv_robot_origin_fixed, 1, cv::Scalar(0,0,255), 1);
        //cv::putText(frame_cv, "Robot origin", cv::Point(cv_robot_origin_fixed.x+6, cv_robot_origin_fixed.y-6), cv::FONT_HERSHEY_COMPLEX, 0.25, cv::Scalar(255,0,0),1);
...
void VideoThread::start()
{
    ...
    if(cv_camera->open(0, cv::CAP_ANY))
    {
        ...
        cv_camera->set(cv::CAP_PROP_FOCUS, 60);
...
void VideoThread::get_ref00_offsets(double x, double y)
{
    ref00_x_off = x;
    ref99_y_off = y;
}
...
//            cv::Point cv_robot_origin = cv::Point(cv_robot_origin_fixed.x+ref00_x_off, cv_robot_origin_fixed.y+ref99_y_off);
//            cv::putText(frame_cv, "(0,0)", cv::Point(cv_robot_origin.x+6, cv_robot_origin.y-6), cv::FONT_HERSHEY_COMPLEX, 0.25, cv::Scalar(255,0,0),1);
//            cv::circle(frame_cv, cv_robot_origin, 3, cv::Scalar(0,0,255));
...
//                    double area = cv::contourArea(contour);
//                    if(area < desired_area+(desired_area/2) && area > desired_area-(desired_area/2)){
//                        cv::approxPolyDP(contour, corners, 0.05*cv::arcLength(contour,true), true);
//                        if(corners.size() == 4){
